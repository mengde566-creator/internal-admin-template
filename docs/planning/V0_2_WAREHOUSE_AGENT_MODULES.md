...
- 03F 经负责人授权后，本模块拥有 `ai_warehouse_search` PostgreSQL 派生索引及其提交后同步/有界对账；它只保存物品编码、名称投影和 1024 维向量，不是仓储事实源。
...
├─→ module-knowledge.api（受控 AI DataSource/EmbeddingModel 公开契约）
...
- SLICE-00已有的仓储操作规则、物品编码规则、仓库与库位编码规则作为初始资产；SLICE-04A在同一目录和导入链上补齐出入库、调拨、盘点及低库存处置等合成制度，不建立第二套样本入口；
- 合成资料必须明确为测试资料，但使用稳定文档编码、版本、生效状态、标题和可引用Markdown章节；至少保留一个失效旧版本，内容规模须支持自然问法、同义问法、零证据、版本过滤和提示注入场景，禁止为单条测试只写一行Demo正文；
...
- 合成资料只能通过上述正式导入路径进入隔离知识库；禁止手工DDL/DML、直接编辑数据库文件、用Liquibase预置向量或绕过Embedding伪造索引完成状态；
- 资料冻结后进行一次有预算的正式Embedding导入与检索验收；日常研发使用Stub覆盖确定性分支，不为调参反复调用Provider或修改留出语料；
- 实时库存、库位余额和出入库流水仅可由Warehouse事实源及隔离业务测试数据提供，不得复制到知识资料中；
