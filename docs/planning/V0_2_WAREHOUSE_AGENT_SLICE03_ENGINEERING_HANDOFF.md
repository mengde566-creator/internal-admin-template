...
> 状态：业务规则已确认；03A、03B、03C、03D、03E、03F 实现与证据均已验收；SLICE-03完成
> 更新日期：2026-08-30
...
- 03E 已完成并复核，出口为 `GATE_PASSED_FOR_03F`；该证据允许进入 03F，但不替代 03F 的生产索引与运行期验证。
...
- 项目负责人已授权在 03E 真实 Gate 通过后直接进入 03F；派生搜索索引由现有 `module-agent-warehouse-adapter` 拥有，不新建模块。
- 索引使用 AI PostgreSQL 中独立的 `ai_warehouse_search` schema、独立 Liquibase changelog 和独立 changelog/lock 表；不读写 `ai_knowledge.*`，不混入 Warehouse 业务迁移。
- `module-knowledge` 只新增一个最小公开 AI 检索基础设施契约，提供当前受控 DataSource 与 1024 维 EmbeddingModel；Adapter 显式依赖该公开 API，禁止按 `knowledgeDataSource`、`knowledgeEmbeddingModel` 等内部 Bean 名称耦合。
- 首版索引文本严格为 `code + " " + name`。现有业务没有确认的别名和稳定描述，禁止凭评估语料或模型生成内容补入生产索引；后续增加别名/描述必须另行确认其业务所有权和维护入口。
- 冻结参数为：`pg_trgm similarity >= 0.45`、topK 3；精确余弦相似度 `>= 0.50`、topK 3；最终候选硬上限 5。禁止 RRF、学习排序、自动阈值确认、HNSW/IVFFlat、独立检索服务和第二套 AI 框架。
- 03F 不改三个按物品 Tool 的模型参数、公共 HTTP/SSE 或前端卡片协议；继续使用 03D 的 `itemMentions`、`excludedItemMentions`、`selectionPreference` 和受信选择链。

### 11.2 Warehouse 公开投影与索引结构

Warehouse 新增窄公开投影 API，不向 Adapter 暴露 Mapper、DO 或表：

1. 按服务端游标稳定扫描物品投影，单页最多 100 条，返回下一游标；必须包含停用物品，供索引写入禁用状态；
2. 按一个物品服务端引用读取最新投影，供提交后事件消费；
3. 按最多 5 个服务端引用和当前 `WarehouseAccessScopeDTO` 批量复核，返回当前仍存在、启用且可见的业务编码、名称与源版本。

投影只包含服务端物品引用、编码、名称、启用状态、源版本和更新时间。服务端引用只用于模块间同步和复核，禁止进入 Tool 参数、模型上下文、卡片、History 或前端。

Adapter 只建立两张自有表：

- `item_search_index`：以“索引版本 + 服务端物品引用”为联合主键，保存编码、名称、启用状态、源版本/更新时间、`search_text`、Embedding 模型/维度、精确向量和索引时间；不建立跨库外键；
- `item_search_sync_state`：当前活动索引版本、正在构建的索引版本、模型/维度、对账游标、最近完整成功时间、状态和最后错误码；只保存同步状态，不保存用户查询。

索引禁止保存库存、仓库/库位余额、流水、用户、部门权限结果、Conversation、Task、Session、密钥、用户原话或查询向量。查询必须参数化；向量列固定 1024 维，首版不建近似索引。

### 11.3 提交后事件与有界对账

- `createItem`、改名、停用和重新启用在业务事务成功提交后发布只含物品引用和源版本的公开变化事件；回滚事务不得发出可消费事件。
- Adapter 事件处理器重新调用 Warehouse 投影 API 取得当前版本。启用物品在数据库事务外生成 `code + name` Embedding，再用短事务 upsert；停用物品写入禁用投影且不参与检索。旧版本 upsert 的 SQL 条件必须保证不能覆盖新版本。
- 单批 Embedding 不超过 20 条；返回数量、模型和 1024 维逐条校验。外部调用失败不得写入该批部分向量，并将同步状态标记为降级。
- 本地事件不是唯一事实源。Agent 开启后异步启动对账，并固定每 5 分钟继续一次：每页 100 条、每轮最多 10 页，持久化下一游标；只为缺失、版本变化、模型/维度或索引版本变化的启用物品生成向量。完成全量一圈后才更新“最近完整成功时间”并从头开始下一圈。
- 重复、乱序事件和对账重叠必须幂等；事件处理和对账读取到的源版本相同则不重复 Embedding。存在构建版本时，提交后事件也写入该构建版本；任一事件/批次失败都阻止切换。模型、维度或索引版本变化时按新版本重建并在整圈成功后原子切换，禁止混合维度；模型或维度已变化时，重建期间相似召回明确降级，不能拿新查询向量检索旧模型索引。
- Agent 关闭时不装配 Adapter 搜索迁移、调度器、事件消费者、DataSource 或 Embedding；人工仓储写操作不等待索引，也不因索引失败而回滚。

### 11.4 运行期检索与用户可见行为

每个未被 03D 解析的物品线索按原始顺序执行同一主路径：

1. 受信 Task/用户选择；
2. Warehouse 编码+名称联合精确；
3. Warehouse 有界前缀/包含；
4. 前三级零候选且搜索索引为当前可用版本时，先查 `pg_trgm`；有候选立即停止；
5. `pg_trgm` 仍零候选时，只对当前原始线索生成一次查询向量并做 pgvector 精确余弦；
6. 按服务端引用去重，取最多 5 个命中回 Warehouse 批量复核；陈旧、停用、越权或版本不一致候选丢弃；
7. 复核后无论只剩 1 个还是多个，相似召回都只生成 `clarification-choice`，不得自动读取库存、位置或流水事实；
8. 用户通过受信 optionToken 确认后，才把当前业务编码交给既有确定性链重新鉴权并查询事实。

多个 `itemMentions` 继续复用 03D 的待处理线索链：只处理当前线索，候选卡保留其余原文；选定后再推进下一条。排除线索在回业务库复核后按当前业务编码/名称应用，模型不能提交服务端引用。三个按物品 Tool（当前库存、物品位置、近期变化）必须共用同一检索编排，禁止只修其中一个。

搜索索引未完成首轮完整对账、版本/维度不符、超过 15 分钟没有完整成功对账，或索引 PG、`pg_trgm`、查询 Embedding 任一失败时：已经成功的精确/字面结果继续返回；只有确实需要相似召回的子任务返回 `AI_RETRIEVAL_DEGRADED`，提示“相似物品检索暂时不可用，请补充名称或编码后重试”。不得返回 `NO_MATCH` 掩盖失败，也不得回退到内存向量或其他数据库。该错误首版不自动加入 03C 重试计划，用户可补充线索或重新发送。

### 11.5 配置、迁移和依赖边界

- 不增加第二套 Provider 配置、模型名或维度；复用已校验的 Qwen `qwen3.7-text-embedding`、1024 维和 AI PostgreSQL选择规则。
- Adapter 可新增对 `module-knowledge` 公开 API 的显式模块依赖，但不新增外部运行时依赖；使用项目已经锁定的 JDBC、Liquibase、Spring AI 和 pgvector 能力。
- Adapter 自有 Liquibase 明确创建 `vector`、`pg_trgm` 扩展及 `ai_warehouse_search` 对象；Agent 已启用但扩展、迁移、模型或维度校验失败时应用启动失败，不静默关闭或回退 SQLite。
- 业务 SQLite + 独立 AI PostgreSQL，以及业务 PostgreSQL 复用同一 AI PostgreSQL，两种拓扑都必须通过正式装配测试；外部/共享数据库仍不得用于研发验证。

### 11.6 观测与安全

- 观测只记录检索阶段（DETERMINISTIC/TRGM/VECTOR/DEGRADED）、耗时、候选数量、索引版本和错误码；不记录用户原话、业务全文、向量、Provider 请求/响应或内部物品引用。
- 查询 Embedding 只发送当前单个物品线索；不得发送整段 History、库存结果、权限、用户或部门信息。
- 每次 Tool 边界重新解析可信 Actor；索引命中永远不能代替 Warehouse 权限与当前状态复核。

### 11.7 最小验收矩阵

1. 创建、改名、停用、重新启用分别证明提交后事件、索引版本和候选变化；业务事务回滚不产生有效索引变化；
2. 重复/乱序事件、事件与对账并发时旧版本不能覆盖新版本，未变化投影不重复调用 Embedding；
3. 丢失一次事件后，有界对账最终修复；分页游标、每轮 10 页上限和完整一圈状态可恢复；
4. 当前库存、物品位置、近期变化分别覆盖错字、俗称、用途描述：相似唯一仍先出候选，选择后才查询事实；
5. 多物品线索的二级候选、剩余线索、排除指代和旧 token/revision 失效继续成立；
6. 陈旧、停用、越权和版本变化命中在回查时被丢弃，服务端引用不进入 Tool/卡片/History；
7. 索引未就绪、过期、数据库失败、pg_trgm 失败和 Embedding 失败时，确定性结果仍可用；需要相似召回时明确 `AI_RETRIEVAL_DEGRADED`，不伪装零结果；
8. 模型/维度/索引版本变化证明新版本完整重建后原子切换；首版无 HNSW/IVFFlat；
9. Agent 关闭时无搜索 Bean/迁移/调度/Provider 调用；两种受支持数据库拓扑完成隔离装配验证；
10. 使用固定桩 Embedding 覆盖实现回归，不重复调用真实 Provider；03E 已冻结的真实 Qwen 基线只作为参数和收益证据。

### 11.8 2026-08-30 实施与复核状态

- 03F 已完成并通过总设计师复核：三个按物品 Tool 共用“确定性零候选 → `pg_trgm` → 仅零候选时 Embedding”的受控链，相似命中经 Warehouse 当前版本和范围复核后只生成候选，不能自动读取事实。
- 派生索引只保存物品编码、名称、源版本、启用状态和1024维向量；创建、改名、停用和启用通过提交后有界异步同步，首轮与每5分钟有界对账负责恢复丢失事件，业务写事务不等待 Embedding。
- 定向回归共54项通过，覆盖陈旧版本丢弃、当前业务字段展示、排除线索、三条 Tool 候选、异步线程、同版本并发去重、对账上限和既有03A～03D行为；Agent启用/关闭与配置回归10项通过。
- 一次无持久卷临时pgvector PostgreSQL集成测试真实执行独立Liquibase、`vector`/`pg_trgm`、索引建表、TRGM、版本更新和停用过滤，测试后容器已销毁；使用固定桩向量，未调用真实Provider或连接共享/外部数据库。
- `AiObservationRecorder`当前只能记录阶段、耗时、错误码和Token，03F没有用字符串拼接伪造候选数或索引版本。候选数与索引版本的结构化观测归SLICE-05完整观测契约统一收口，不反向扩大03F公共API。
- 以上证据关闭SLICE-03；不表示SLICE-04知识问答、混合知识引用或SLICE-05完整观测已经完成。
