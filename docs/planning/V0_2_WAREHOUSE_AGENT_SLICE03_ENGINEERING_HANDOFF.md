...
- 建立版本化、可审阅的自然语言评估集，至少覆盖：单字替换、漏字、颠倒字、大小写/符号、简称、现场俗称、用途描述、同名、短编码、编码与名称冲突、无关描述和恶意物品名。
- 每条只记录用户原话、正确候选集合、不得自动选择集合和期望 topK，不复制真实库存、权限、用户或敏感数据。
- 指标至少包括 exact recall、typo topK recall、semantic topK recall、候选数量、零结果保持率和错误自动选择次数；错误自动选择必须始终为 0。
- Gate 阶段只允许隔离 PoC/测试资产，不修改正式 Tool、生产契约或正式 Liquibase，不建立后台同步链。调用真实 Embedding Provider 前仍需单独授权；可以先用固定桩向量验证编排。
...
2. `pg_trgm` 或 pgvector 能显著提高目标 topK，且不会错误自动选择；
3. 根据实验冻结首版 topK、候选上限、最低相似度和合并规则，不能留给研发凭经验填写；
4. 明确需要 `pg_trgm`、pgvector 两者还是其中一个。若字符召回已满足当前缺口，不为“以后可能需要”强行上线向量；
5. 项目负责人接受派生索引所有权、跨库最终一致性和运行期 Embedding 依赖。
