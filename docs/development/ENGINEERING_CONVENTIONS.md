...
新建接口或实质改变既有接口时，按 [INTERFACE_CONTRACT.md](../ai/templates/INTERFACE_CONTRACT.md) 在当前任务中核对差值：
...
4. 默认不创建永久接口清单；长期业务语义或跨模块公共契约写回需求或模块 `CAPABILITY.md`。
...
7. 视觉交互：涉及 UI 时，是否使用真实业务状态，在需求确认的内容宽度和系统实际主题下走查，并覆盖会改变布局的代表性动态数据？禁止用空数据截图或组件存在性代替。
...
> 机制落点：AGENTS.md §16.2 第 5-8 条（强制红线）+ requirements/README.md §8（轻量需求漏斗）。
...
- 涉及新功能/UI 变更/表结构的需求，实现前必须走需求漏斗（requirements/README.md §8）；
- UI 需求必须确认：页面类型与气质（营销/信息/工具型）、参考方向、参数偏好；
- 含糊需求词（"铺满""好看""大气"）必须澄清为可确认的选项，禁止直接动手。

## 9. 复杂链路的结构化诊断日志（2026-09-08 确认）

- 异步处理、文件解析、外部调用、跨事务或多阶段状态机等复杂链路，必须在阶段边界留下可关联的结构化日志：关联标识、阶段、结果或稳定错误码，以及必要的耗时/计数。
- 日志只记录定位首个偏差层所需的脱敏元数据；禁止记录文件正文、完整单元格值、路径、敏感凭据和异常堆栈。INFO 记录关键生命周期与终态，DEBUG 记录受控细节，WARN 记录可预期失败，ERROR 仅用于需要人工处置的一致性或补偿失败。
- 日志用于辅助定位，不替代业务测试；简单 CRUD 不机械增加阶段日志。临时调试输出必须在交付前删除。
