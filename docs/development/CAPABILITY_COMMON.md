...
├─ AI_PROMPT.md   # 模块特有：定位、约束、踩坑表、禁止事项（通用规则指向本文件）
├─ CONTRACT.md    # 模块特有：公开 API 契约 + 表/DO/DTO（合并原 DESIGN + DATA_CONTRACT）
└─ TEST.md        # 模块特有必测用例（通用用例指向本文件第 3/4 节）
```

- AI 开发模块前必须加载 `AI_PROMPT.md`（AGENTS §2.3）；
