...
## V01-12：完整运行验证（已通过，且未由本生成链替代）

下列验证不属于无数据库契约上下文，已由 V01-12 在项目负责人允许的隔离环境执行：Java 25、最终 fat jar、完整主应用上下文、真实 Security 链、Liquibase/SQLite、`OpenApiContractTest` 与端到端运行验证。固定 SHA `90d4d313b3483f07237a6e0cf342953d980f20f5` 的运行时 OpenAPI 1/1、完整质量、真实 Chromium 3/3 与远端 Actions 均通过；V01-05 的生成证据仍不能被单独描述为这些完整运行证据。
