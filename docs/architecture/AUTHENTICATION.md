...
`/api/public/**` 是模板级匿名命名空间，不代表其中每个路径天然公开。进入该命名空间的生产 Controller 必须主动声明匿名访问规则，并只返回已确认允许公开的数据；任何裁剪或派生操作都必须检查是否仍有遗留公开 Controller，以及其引用的数据是否仍具备发布可见性校验。内部管理接口不得借用该命名空间绕过 Session 或权限校验。
