...
public enum AgentErrorCode implements ErrorCodeContract {
    ...
    TOOL_DATABASE_UNAVAILABLE("AI_TOOL_DATABASE_UNAVAILABLE", "数据源暂时不可用"),
    TOOL_EXECUTION_FAILED("AI_TOOL_EXECUTION_FAILED", "工具执行暂时未完成"),
    ARTIFACT_INVALID("AI_ARTIFACT_INVALID", "工具中间结果无效"),
    TOOL_CALL_BATCH_INVALID("AI_TOOL_CALL_BATCH_INVALID", "本轮只允许一个工具调用"),
    KNOWLEDGE_UNAVAILABLE("AI_KNOWLEDGE_UNAVAILABLE", "知识库暂时不可用，请稍后重试。"),
    RETRIEVAL_DEGRADED("AI_RETRIEVAL_DEGRADED", "检索暂时不可用，请补充查询条件后重试"),
