...
- 写完立即自查（ENGINEERING_CONVENTIONS §3）；验证时显式选择 `./scripts/quality.sh --no-database` 或 `./scripts/quality.sh --database`：后者先执行前者，再执行隔离 SQLite 集成与空库启动验证。后端使用 Maven Wrapper，前端与 OpenAPI 工具依赖以 `npm ci` 按锁文件安装；V01-08 的 `SiteFlowTest`、V01-10 质量链与固定 SHA 的 V01-12 发布级验证均已验收。
