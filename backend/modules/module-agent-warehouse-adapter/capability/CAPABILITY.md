...
SLICE-07A 后，本模块通过编译期 `AgentAdapter` 契约向 Core 注册仓储 Tool、可用性、受信提示、Task Policy、卡片类型和路由键；仓储意图、候选、澄清、恢复和卡片校验由本模块拥有，Core 不提供默认仓储 Adapter 或仓储分支。当前声明只用于静态能力与所有权；Run 内 ToolArtifact 的具体 Tool 级生产/消费协议尚未实现，属于 07B。
