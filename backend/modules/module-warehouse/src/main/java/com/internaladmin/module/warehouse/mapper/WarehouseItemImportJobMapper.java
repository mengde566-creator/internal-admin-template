...
public interface WarehouseItemImportJobMapper extends BaseMapper<WarehouseItemImportJobDO> {
    ...
    @Select("SELECT job_id, client_request_id, creator_user_id, file_asset_id, file_sha256, status, revision, created_at, expires_at, updated_at, error_code, total_rows, create_count, update_count, disable_count, unchanged_count, invalid_count, conflict_count FROM (" +
            "SELECT job_id, client_request_id, creator_user_id, file_asset_id, file_sha256, status, revision, created_at, expires_at, updated_at, error_code, total_rows, create_count, update_count, disable_count, unchanged_count, invalid_count, conflict_count, " +
    ...
    @Update("UPDATE wh_item_import_job SET status='ANALYZING', revision=revision+1, updated_at=#{updatedAt}, error_code=NULL WHERE job_id=#{jobId} AND creator_user_id=#{userId} AND revision=#{revision} AND status='RECEIVED' AND expires_at > #{updatedAt}")
